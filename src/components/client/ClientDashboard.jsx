...
import { User, Scale, Activity, Droplets, Calendar, MessageSquare, Plus, CheckCircle2, Award } from 'lucide-react';

export default function ClientDashboard() {
  const { currentUser } = useAuth();
  const [activeTab, setActiveTab] = useState('plan'); // 'plan' | 'progress' | 'messages'
      ...
      <div className="glass-panel rounded-3xl p-6 sm:p-8 border border-emerald-500/30 shadow-2xl flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="gradient-btn-emerald px-5 py-2.5 rounded-xl text-xs font-bold flex items-center gap-2"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="glass-panel rounded-2xl p-5 border border-white/10 flex items-center justify-between">
          <div>
            ...
            <p className="text-[11px] text-emerald-400 font-medium mt-0.5">Haftalık %0.8 Azalma</p>
          </div>
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          {activeTab === 'plan' ? (
            ...
            <div className="glass-panel rounded-3xl p-6 sm:p-8 border border-white/10 shadow-2xl space-y-6">
              <h3 className="text-lg font-bold text-white">Aylık Kilo & Yağ Oranı Değişimi</h3>
              
              {/* Mock Graphical Bar Chart */}
              <div className="h-64 flex items-end justify-between gap-4 pt-8 pb-4 border-b border-white/10 px-4">
                {[
                  { month: '1. Hafta', weight: 78.5, fat: 29.2 },
                  { month: '2. Hafta', weight: 76.2, fat: 28.0 },
                  { month: '3. Hafta', weight: 74.0, fat: 27.1 },
                  { month: '4. Hafta (Bugün)', weight: 71.2, fat: 26.4 }
                ].map((item, idx) => (
                  <div key={idx} className="flex-1 flex flex-col items-center gap-2 h-full justify-end">
                    <div className="w-full flex justify-center gap-1.5 items-end h-full">
                      {/* Weight bar */}
                      <div
                        className="w-8 rounded-t-xl bg-gradient-to-t from-emerald-600 to-emerald-400 text-[10px] font-bold text-slate-950 flex items-center justify-center pt-2 transition-all hover:brightness-110"
                        style={{ height: `${(item.weight / 90) * 100}%` }}
                      >
                        {item.weight}
                      </div>
                      {/* Fat % bar */}
                      <div
                        className="w-8 rounded-t-xl bg-gradient-to-t from-teal-600 to-teal-400 text-[10px] font-bold text-slate-950 flex items-center justify-center pt-2 transition-all hover:brightness-110"
                        style={{ height: `${(item.fat / 40) * 100}%` }}
                      >
                        %{item.fat}
                      </div>
                    </div>
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <div className="glass-panel rounded-3xl p-6 border border-white/10 shadow-xl space-y-4">
            ...
            <div className="flex items-center space-x-3 p-3 rounded-2xl bg-slate-900/80 border border-white/5">
              ...
              />
              <div>
                <h4 className="text-sm font-bold text-white">Dyt. Zeynep Kaya</h4>
                <p className="text-[10px] text-emerald-400 font-medium">Klinik Beslenme Uzmanı</p>
              </div>
            </div>

            <button
              onClick={() => alert("Diyetisyeninize mesaj iletildi! En kısa sürede dönüş sağlanacaktır.")}
              className="w-full py-2.5 rounded-xl bg-slate-800 text-slate-200 hover:bg-slate-700 text-xs font-bold flex items-center justify-center gap-2 border border-white/5"
            >
